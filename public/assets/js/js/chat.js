...
function chatHeartbeat(){
	...
	jQuery.ajax({
	  url: "http://soon.anekapaperaindah.id/chat.php?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
...
function closeChatBox(chatboxtitle) {
	...
	jQuery.post("http://soon.anekapaperaindah.id/chat.php?action=closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		jQuery.ajax({
			url: "http://soon.anekapaperaindah.id/chat_controller/chat_count",
			//cache: false,
			//dataType: "json",
			success: function(data) {
				//alert(data);
				...
				if (data == 'failed'){
					jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxinfo">Chat disabled, please try again after a few minutes</span></div>');
				}else if(data == 'block'){
					jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxinfo">You can not use this live chat</span></div>');
				}else{
					...
					if (message != '') {
						jQuery.post("http://soon.anekapaperaindah.id/chat.php?action=sendchat", {to: chatboxtitle, message: message} , function(data){
...
function startChatSession(){  
	jQuery.ajax({
	  url: "http://soon.anekapaperaindah.id/chat.php?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
